...
data = sc.textFile("hdfs:///HW1/output.csv")
...
while not converged and n <limit:
    n+=1
    clusters = data.map(lambda point: assign_to_nearest_centroid(point, centroids_broadcast.value))
    ...
    new_centroids_list = [centroid for (_, centroid) in new_centroids]
    # Check for convergence
    converged = all([euclidean_distance(new_centroids_list[1], centroids_broadcast.value[i]) < 1e-8 for i in range(k)])
    centroids_broadcast.unpersist()
    centroids_broadcast = sc.broadcast(new_centroids_list)
